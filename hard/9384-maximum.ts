...
import { LessThan } from './9155-valid-date';

type FindMaximum<T, U extends number = 0> = T extends [infer First extends number, ...infer Rest]
  ? LessThan<First, U> extends true
  ? FindMaximum<Rest, U>
  : FindMaximum<Rest, First>
  : U;

type Maximum<T extends number[]> = T extends [] ? never : FindMaximum<T>;

// https://github.com/type-challenges/type-challenges/issues/21928
