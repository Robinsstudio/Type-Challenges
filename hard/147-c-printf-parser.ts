...
type ParsePrintFormat<T> = T extends `${string}%${infer First extends keyof ControlsMap}${infer Rest}`
  ? [ControlsMap[First], ...ParsePrintFormat<Rest>]
  : T extends `${string}%%%${infer First extends keyof ControlsMap}${infer Rest}`
  ? [ControlsMap[First], ...ParsePrintFormat<Rest>]
  : [];

// https://github.com/type-challenges/type-challenges/issues/20510
