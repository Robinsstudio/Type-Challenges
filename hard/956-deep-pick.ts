...
type DeepPickUnion<T, K extends string> = K extends keyof T
  ? Record<K, T[K]>
  : K extends `${infer First extends Extract<keyof T, string>}.${infer Rest}`
  ? Record<First, DeepPickUnion<T[First], Rest>>
  : never;
