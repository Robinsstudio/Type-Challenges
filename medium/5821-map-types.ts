...
type Extract<K, R extends { mapFrom: unknown, mapTo: unknown }> = R extends infer First extends R
  ? (
    K extends First['mapFrom']
      ? R['mapTo']
      : never
  ) | Extract<K, Exclude<R, First>>
  : never;

type MapTypes<T, R extends { mapFrom: unknown, mapTo: unknown }> = {
  [K in keyof T]: T[K] extends R['mapFrom'] ? Extract<T[K], R> : T[K]
};
