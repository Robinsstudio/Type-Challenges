...
> = T extends `${infer First}${infer Rest}`
  ? First extends '-' | '+'
  ? PercentageParser<Rest, [First, Output[1], Output[2]]>
  : First extends `${number}`
  ? PercentageParser<Rest, [Output[0], `${Output[1]}${First}`, Output[2]]>
  : First extends '%'
  ? PercentageParser<Rest, [Output[0], Output[1], First]>
  : PercentageParser<Rest, Output>
  : Output;

// https://github.com/type-challenges/type-challenges/issues/21114
// type PercentageParser<A extends string, Sign = '', Unit = ''> = A extends `${infer Left}%`
//   ? PercentageParser<Left, '', '%'>
//   : A extends `${infer S extends '+' | '-'}${infer Right}`
//   ? PercentageParser<Right, S, Unit>
//   : [Sign, A, Unit]
